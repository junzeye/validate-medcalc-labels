...
@hydra.main(config_path="conf", config_name="ppo_trainer", version_base="1.3")
def main(config):
    run_ppo(config)
...
class TaskRunner:
    def run(self, config):
        ...
        # Suppress repetitive Ray logging
        logging.getLogger("ray").setLevel(logging.WARNING)

        print(f"TaskRunner hostname: {socket.gethostname()}, PID: {os.getpid()}")

        # Only print full config if explicitly requested
        if config.get("debug", {}).get("print_full_config", False):
            pprint(OmegaConf.to_container(config, resolve=True))
        else:
            print("Training configuration loaded (use debug.print_full_config=true to see full Hydra config)")
