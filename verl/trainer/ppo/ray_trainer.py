...
# from datetime import datetime # previously used for unique experiment name -> now this is handled by Hydra
...
from verl.trainer.ppo.metric_utils import (
    ...
    tony_process_validation_metrics,
)
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        # DEBUG: Check what n value is being used
        val_rollout_size = self.config.actor_rollout_ref.rollout.val_kwargs.n
        print(f"DEBUG: for validation rollout, val_kwargs.n = {val_rollout_size}")

        # Can probably wrap the following in a tqdm to track progress
        for test_data in self.val_dataloader:
            ...
            # NOTE: deemed unnecessary for now - Tony
            ...
            # for DEBUG - Tony
            print(f"DEBUG - validation reward_tensor shape: {reward_tensor.shape}")
            print(f"DEBUG - validation scores shape: {len(scores)}")
        ...
        data_src2var2metric2val = tony_process_validation_metrics(data_sources, sample_inputs, reward_extra_infos_dict, val_rollout_size)
        # data_src2var2metric2val = process_validation_metrics(data_sources, sample_inputs, reward_extra_infos_dict)
        metric_dict = {}
        for data_source, var2metric2val in data_src2var2metric2val.items():
            for var_name, metric2val in var2metric2val.items():
                for metric_name, metric_val in metric2val.items():
                    if (var_name in ["score", "acc", "format_acc"]) and any(metric_name.startswith(pfx) for pfx in ["mean", "maj", "best"]) and ("@1" in metric_name):
                        metric_sec = "val-core"
                    else:
                        metric_sec = "val-aux"
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            ...
            if self.config.trainer.get("val_only", False): # Validation only -> after validation, exit
                return
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if do_profile:
                    ...
                    if self.use_critic: # NOTE: `use_critic` is set to False automatically, as `adv_estimator=grpo` in config.
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        # compute rewards. apply_kl_penalty if available - not applicable for GRPO, so the reward is the same as the score.
