...
class ActorRolloutRefWorker(Worker, WorkerProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if torch_dtype:
            print(f"Loaded model in {torch_dtype}! Param detected.") # TODO: debug use, remove later - by Tony
        
        if torch_dtype is None:
            torch_dtype = torch.bfloat16 if self._is_actor else torch.bfloat16 # NOTE: Tony changed this from float32 to bfloat16. Was float32 for actor
        else:
        ...
        if self.rank == 0:
            # Only print essential model info, not full config
            print(f"Model: {actor_model_config.model_type}, Hidden size: {actor_model_config.hidden_size}, Vocab size: {actor_model_config.vocab_size}")
