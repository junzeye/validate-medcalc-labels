...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: Union[str, List[str]] = "console", config=None):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            settings = wandb.Settings(x_stats_sampling_interval=config.get("trainer", {}).get("wandb_stats_sample_rate", 15))
            if config and config["trainer"].get("wandb_proxy", None):
                settings = wandb.Settings(https_proxy=config["trainer"]["wandb_proxy"],x_stats_sampling_interval=config.get("trainer", {}).get("wandb_stats_sample_rate", 15))
