# Validating MedCalc-Bench Labels with Physician Oversight
...
This repository contains the data artifacts and code from our case study of scalable benchmark stewardship using [MedCalc-Bench](https://openreview.net/pdf?id=VXohja0vrQ), a widely used clinical benchmark that was published as a peer-reviewed dataset (NeurIPS 2024 Oral) for evaluating LLMs on clinical score computation. In this repository, we present a physician-in-the-loop stewardship pipeline that combines agentic LLM verifiers with automated triage to concentrate scarce clinician attention on contentious instances. Four board-licensed physicians from three specialties at Stanford Medicine provided valuable feedback and oversight in our study.
